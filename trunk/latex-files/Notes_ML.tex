...
\begin{equation}
\colorboxed{navy}{~ f_*|X,y,X_* \sim \mathcal{N}\bepar{\overline{f}_*, \textsc{cov}\left (f_*\right )}~}
\end{equation}
...
\end{align}
La moyenne $\overline{f}_*$ peut être vue comme une combinaisons linéaire des observables \textbf{y}. En effet, on peut écrire  l'équation \eqref{mean_pred} en considérant $n$ produits scalaires chacun centré autour d'un point du training set :
\begin{equation}
...
On introduit alors la fonction de coût $\mathcal{L}\bepar{y_{\text{true}}, y_{\text{guess}}}$ que l'on va chercher à minimiser. Cependant, non ne connaissons pas a priori la valeur de $y_{\text{true}}$. \\
