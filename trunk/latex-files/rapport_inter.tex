...
Le premier cas sur lequel nous avons utilisé l'inférence est sur un problème thermique provenant de \citep{parish2016paradigm}.\\
Le but est d'établir l'évolution du champ de température dans un barreau dont les températures au début et en fin de domaine sont nulles. Cette évolution se fait sous des termes de conduction, de convection et de radiation.\\
...
\begin{equation}
 ...
 \end{equation}
 On commence par résoudre ce problème exact pour plusieurs tirage de $\varepsilon$. En même temps de construire une dataset de référence, on évalue la sensibilité des résultats par rapport à la distribution $\mathcal{N}$ présente dans l'équation \eqref{epsilon}. Cette sensibilité se traduit par la construction d'une matrice de covariance des données observables issues de ce problème exact.\\
...
\end{equation}
Nous construisons la $\covpri$ en diagonalisant des valeurs fournies par \citep{parish2016paradigm}.\\
Nous minimisons alors la fonction de coût pour trouver $\bmap$ selon la formule \eqref{bmapJ}. On compare ensuite 
