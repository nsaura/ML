...
#            T_n    =   A2@T_n
            T_n_2_tmp  =   np.dot(A2, T_n_2)
             
            for i in range(N_discr-2) :
                B_n[i] = T_n_tmp[i] +dt*((10**(-4)*(1.+5.*np.sin(3.*T_n[i]*np.pi/200.) + np.exp(0.02*T_n[i]) + bruit[i]))*(T_inf**4-T_n[i]**4)+h*(T_inf-T_n[i]))
                   
                B_n_2[i] =T_n_2_tmp[i]+dt*5*10**(-4)*(T_inf**4-T_n_2[i]**4)*(1+bruit[i]) 
