...
#        np.random.seed(1000) ; #plt.ion()
        ...
        self.err_title = osp.join("err_check", "LIN_%s_err_check.csv" %(self.date))
        self.logout_title = osp.join(parser.logbook_path, "LIN_%s_logbook.csv" %(self.date))
        ...
        for t in self.T_inf_lst :
            ...
            if self.bool_method["opti_scipy_%s" %(sT_inf)] :
                for f, s in zip([self.betamap, self.cholesky],["beta", "cholesky"]) :
                    path_tosave = "LIN_opti_scipy_%s_%s_N%d_cov%s.csv" %(s, sT_inf, self.N_discr-2, self.cov_mod)
            ...
            if self.bool_method["adj_bfgs_%s" %(sT_inf)] :
                for f, s in zip([self.bfgs_adj_bmap, self.bfgs_adj_cholesky],["beta", "cholesky"]) :
                    path_tosave = "LIN_adj_bfgs_%s_%s_N%d_cov%s.csv" %(s, sT_inf, self.N_discr-2, self.cov_mod)
                    path_tosave = osp.join(self.path_fields, path_tosave)
                    self.pd_write_csv(path_tosave, f[sT_inf])
...
#   Initial condition
        ...
        err, tol, compteur, compteur_max = 1., 1e-7, 0, 10000
        if verbose == True :
            plt.figure()
        ...
        while (np.abs(err) > tol) and (compteur <= compteur_max) :
            ...
            for i in range(self.N_discr-2) :
                try :
                    B_n[i] = T_n_tmp[i] + self.dt*(beta[i])*self.h*(T_inf - T_n[i])
                except IndexError :
                    print ("i = ", i)
                    print ("B_n = ", B_n)
                    print ("T_n = ", T_n)
...
##---------------------------------------------------
    def true_beta(self, T, T_inf) : 
        ...
        eps = np.asarray([ (1. + 5.*np.sin(3.*np.pi/200. * T[i]) + np.exp(0.02*T[i])) *10**(-4) for i in range(self.N_discr-2)])
        res = np.asarray([1. + eps[i]/self.h*(T_inf**4 - T[i]**4)/(T_inf - T[i])  for i in range(self.N_discr-2)]) 
        return res 
##---------------------------------------------------    
##----------------------------------------------------##
##----------------------------------------------------##
    ...
    def obs_pri_model(self) :
        ...
        for T_inf in self.T_inf_lst :
            for it, bruit in enumerate(self.lst_gauss) :
                ...
                obs_filename  =  'LIN_{}_obs_T_inf_{}_N{}_{}.csv'.format(self.cov_mod, T_inf, self.N_discr-2, it)
                pri_filename  =  'LIN_{}_pri_T_inf_{}_N{}_{}.csv'.format(self.cov_mod, T_inf, self.N_discr-2, it)
...
#                zip_ref.close()
                ...
                tol ,err_obs, err_pri, compteur = 1e-7, 1.0, 1.0, 0                
...
#                T_n_pri =  list(map(lambda x : -4*T_inf*x*(x-1), self.line_z) ) # On va utiliser le modèle incomplet -> pri
                ...
                while (np.abs(err_obs) > tol) and (compteur < 8000) and (np.abs(err_pri) > tol):
                    if compteur > 0 :
                    ...
                    for i in range(self.N_discr-2) :
                        ...
                        B_n_pri[i] = T_n_pri_tmp[i] + self.dt * (self.h*T_inf - T_n_pri[i]) * (1 + bruit[i])
...
##----------------------------------------------------##
    def DR_DT(self, beta, T_inf) :
        INF = np.diag([(self.N_discr-1)**2 for i in range(self.N_discr-3)], -1) #Ex inf
        SUP = np.diag([(self.N_discr-1)**2 for i in range(self.N_discr-3)], +1) #Ex sup
        A_diag = - np.diag([beta[i] * self.h + 2 * (self.N_discr-1)**2 for i in range(self.N_discr-2)]) #Diag

        result = A_diag + INF + SUP
...
#        print ("DR_DBETA in function -- h_beta**4 =\n {} \n T_inf**4 = {}".format((T_inf**4 - self.h_beta(beta, T_inf)**4) * T.eps_0, T_inf**4))
        return np.diag((T_inf - self.h_beta(beta, T_inf))* self.h)
##----------------------------------------------------##
