...
def wolf_conditions(f, df, xk, dk, strong=True , verbose = False,c1 = 1e-4, c2 = 0.1, alpha=0.1):
    ...
    if strong== True :  
        while (False in t1(alpha) or t2(alpha) == False ) and cpt < 100 :
            cpt +=  1
        ...
        while (False in t1(alpha) or False in t2(alpha)) and cpt < 100 :
            cpt +=  1
            if cpt % 10 ==0 and verbose== True :
                print("cpt : {}\nt1 = {} \t t2 = {}".format(cpt, t1(alpha), t2(alpha)))
            alpha *= 0.5
...
#    wolf_conditions(f, df, xk, dk, strong=True ,c1 = 1e-4, c2 = 0.1, alpha=0.1):
    alpha = wolf_conditions(J, g_J, x_n, d_n, strong = False, verbose=False ,alpha=1.)
