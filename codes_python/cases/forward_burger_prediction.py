...
def forwrad_compute_true_u(cb, nsamples, pi_line, kcmax = 1, plot=False, write=False) :
    X = np.zeros((cb.Nx))
    y = np.zeros((cb.Nx))
...
dict_layers = {"I": cb.Nx, "O" :cb.Nx, "N1":100, "N2":100, "N3":100, "N4":100, "N5":100, "N6":100, "N7":100}#, "N7":80, "N8":80, "N9":80, "N10":80}#, "N11":40}
...
# run forward_burger_prediction.py -nu 2.5e-2 -itmax 80 -CFL 0.4 -num_real 5 -Nx 52 -Nt 32 -beta_prior 10 -typeJ "u"
# X_forward, y_forward = forwrad_compute_true_u(cb, 12, pi_line, plot=True, write=True)
# nn_forward = forward_multi_buildNN(1e-3, X_forward, y_forward, "selu", "Adam", "MSEGrad", 70, "sum", "Standard", N_=dict_layers, color="purple",  bsz=64,  BN=True)
# forward_multiNN_solver(nn_forward)
