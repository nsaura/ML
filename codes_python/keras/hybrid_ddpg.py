...
#----------------------------------------------
def save_weights() :
    act.target_model.save_weights("./ACweights/actmodelsecond.h5", overwrite=True)
    cri.target_model.save_weights("./ACweights/crimodelsecond.h5", overwrite=True)
    print("Saved")
#----------------------------------------------
def load_weights() :
    act.model.load_weights("./ACweights/actmodelsecond.h5")
    cri.model.load_weights("./ACweights/crimodelsecond.h5")
    
    act.target_model.load_weights("./ACweights/actmodelsecond.h5")
    cri.target_model.load_weights("./ACweights/crimodelsecond.h5")
    print("Loaded")
#----------------------------------------------
...
if osp.isfile("./ACweights/actmodelsecond.h5") and osp.isfile("./ACweights/crimodelsecond.h5") :
    print("act and cri weights removed")
    remove("./ACweights/actmodelsecond.h5")
    remove("./ACweights/crimodelsecond.h5")
save_weights()
...
def play_with_burger(u_init):    
    ...
    file = open("play_rewardsecond.txt", "w")
    file.close()
...
#        plt.pause(0.01)
        ...
        r_t = reward(s_t1, s_t)
        file = open("play_rewardsecond.txt", "a")
        file.write("%.4f\n" %(r_t))
        file.close()
...
def train (u_init, play_type="AC") :
    ...
    file = open("rewardssecond.txt", "w") ; file.close()
    file = open("delta_maxsecond.txt", "w") ; file.close()
    
    s_a_file = open("state_actionsecond.txt", "w") ; s_a_file.close()
...
#            print ("dones shape  : ", dones.shape)
            ...
            with graph.as_default():
                ...
                s_a_file = open("state_actionsecond.txt", "w")
...
##                    print deque_obj.size()
        ...
        plt.figure("Evolution de Loss sur un episodes vs iteration STEP = 10")
