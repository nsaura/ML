...
%Si on avait utilisé un autre filtre de dimension $5\times5\times 3$, on aurait eu une feature map de dimension ($28$, $28$, $2$). En utilisant plus de filtres, on est capable de mieux préserver les dimensions spatiales.\\
%Un example des différentes couches possibles. Des explications sur les différents termes utilisés suivront

\navy \subsubsection{Image alignment} \bk
...
Suppose we try to aligne template image $T(\textbf{x})$ to an input image $I(\textbf{x})$, where $\textbf{x} = (x,y)^T$ column vector containing the pixel coordinates. Let $\textbf{W}(\textbf{x}; \textbf{p})$ denote the warp (distorsion, manipulation digitale d'image) and $\textbf{p} = (p_1,\, .\, .\, ., p_n)^T$ a vector of parameters.\\

The goal of image alignment is to find the $\textbf{\textit{p}}^*$ that minimizes the distance between the template $T(\textbf{\textit{x}})$ and the input image, warped back onto the coordinate frame of the tenplate $\textbf{I}(\textbf{\textit{W}}(\textbf{\textit{x}}; \textbf{\textit{p}}))$ :
\begin{equation*}
...
\end{equation*}
This minimization is made through iterative methods : each iteration ends with $\textbf{\textit{p}} \leftarrow \textit{\textbf{p}} + \Delta \textbf{\textit{p}}$. This process is stopped when $\left\vert\left\vert \Delta \textbf{\textit{p}} \right\vert\right\vert  < \varepsilon$ 
